# Album Clip Timeline Builder v1.0.0

Album Clip Timeline Builder は、複数の音声クリップ（WAV / MP3）をアルバム形式のタイムラインへ並べ、実音開始・実音終了を基準に曲間を整えたうえで、アルバムWAV、結合SRT、タイムシート、オフセット表を出力するローカルHTMLアプリです。
...
6. Export `album.wav`.
7. Optionally export `album_subtitles.srt`, `offset_image.txt`, `timesheet.txt`, and `project.json`.
